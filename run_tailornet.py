...
OUT_PATH = "/home/scott/graphics_project/out"
...
def get_sequence_inputs(garment_class, gender):
    ...
    beta = get_specific_shape(sys.argv[5])
    if garment_class == 'old-t-shirt':
        gamma = get_specific_style_old_tshirt('big_longsleeve')
    else:
        gamma = get_style(sys.argv[4], gender=gender, garment_class=garment_class)

    # downsample sequence frames by 2
    thetas = get_amass_sequence_thetas(sys.argv[6])[::2]
